 ...
 * @since  1.6
 */
class Rw_accountsModelDomains extends \Joomla\CMS\MVC\Model\ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		if (!Factory::getUser()->authorise('core.edit', 'com_rw_accounts'))
		{
            ...
            if (!empty($search))
            {
                ...
                else
                {
					...
					$query->where('( a.code LIKE ' . $search . '  OR  a.areaname LIKE ' . $search . '  OR  a.groupname LIKE ' . $search . '  OR  a.domain LIKE ' . $search . '  OR  a.web_master LIKE ' . $search . ' )');
                }
            }
	...
	public function getItems()
	{
		...
		foreach ($items as $item)
		{
			...
			$item->status = JText::_('COM_RW_ACCOUNTS_DOMAINS_STATUS_OPTION_' . strtoupper($item->status));
		}
	...
	protected function loadFormData()
	{
		...
		if ($error_dateformat)
		{
			$app->enqueueMessage(Text::_("COM_RW_ACCOUNTS_SEARCH_FILTER_DATE_FORMAT"), "warning");
